...
from utils.data import Dataset, collate_fn, NERDataset, ner_collate_fn, BartSeq2SeqDataset, bartseq2seq_collate_fn, BartTxtSeq2SeqDataset, barttxtseq2seq_collate_fn
...
from wenet.transformer.asr_model import init_ctc_model, init_asr_model
from model.ctc_bert import CTCBertModel, CTCBertNERModel
from utils.process import read_symbol_table
...
class CTCBertNERParser(object):
    ...
    def load_model(self):
        use_cuda = self.args.device != '-1' and torch.cuda.is_available()
        device = torch.device('cuda' if use_cuda else 'cpu')
        self.model.load_state_dict(torch.load(os.path.join(self.args.path, 'best.model'), map_location=device))
        self.model = self.model.to(device)
    ...
    def eval(self, dataloader=None):
        ...
        if dataloader is None:
            ...
            logger.info(f"test:{len(data_set):6}\n")


        metric = ChartMetric()
        bar = progress_bar(dataloader)
        start = datetime.now()
        with open(self.args.res, 'w', encoding='utf8') as f:
            for i, dic in enumerate(bar, 1):
                audio_feat = dic['audio_feat']
                asr_target = dic['asr_target']
                feats_lengths = dic['audio_feat_length']
                ...
                for sent_idx in range(pred.shape[0]):
                    f.write(f'snt: {keys[sent_idx]}\n')
                    if pred[sent_idx].gt(-1).sum() > 0:
                        f.write(f'nes: ')
                        this_res = []
                        for ed, st in pred[sent_idx].gt(-1).nonzero().tolist():
                            sent_span = keys[sent_idx][st-1: ed]
                            ne_label = self.ner_vocab.itos[pred[sent_idx][ed, st].item()]
                            this_res.append(sent_span+':'+ne_label)
                        f.write(' '.join(this_res)+'\n')
                    else:
                        f.write(f'nes: 无\n')
                    f.write('\n')
